...
void alphaBlend_c(int *fgImage, int *bgImage, int *dstImage)
{
    ...
    for(x=0; x< (512*512)/8; x++)
    {	
        uint8x8_t const_255 = vdup_n_u8 ((uint8_t)255);		//loading a double reister to hold 8 elements of value 255 for taking the complement of the alpha
        result.val[3]=const_255;			//first byte of dest image is 255
        fg_rgb = vld4_u8((const uint8_t *)&fgImage[x*8]);
        bg_rgb = vld4_u8((const uint8_t *)&bgImage[x*8]);
        ...
        result.val[0] = vshrn_n_u16(temp, 8);						//divide by 256
        ...
        result.val[1] = vshrn_n_u16(temp, 8);						//divide by 256
        ...
        result.val[2] = vshrn_n_u16(temp, 8);						//divide by 256

        vst4_u8 ((uint8_t *)&dstImage[x*8], result);				//send the data back to the destination array    	  
    }
}
